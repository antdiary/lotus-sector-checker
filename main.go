...
var log = logging.Logger("lotus-sector-checker")
...
func main() {
	...
	log.Info("Starting lotus-sector-checker")

	app := &cli.App{
		Name:    "lotus-sector-checker",
		Usage:   "Sector checker performance of lotus on your hardware",
		...
		go func(worker int) {
			sealerr := func() error {
				...
				for i := abi.SectorNumber(start); i < abi.SectorNumber(end); i++ {
					...
					pc1o, err := sb.SealPreCommit1(context.TODO(), sid, ticket, 0, pieces)
					if err != nil {
					...
					cids, err := sb.SealPreCommit2(context.TODO(), sid, pc1o[0:pc1olen-64])
					if err != nil {
						return xerrors.Errorf("commit: %w", err)
					}
